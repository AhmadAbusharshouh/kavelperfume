...
export default function CartPage() {
  ...
  // Set default area when governorate changes
  useEffect(() => {
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store order in local memory for immediate success receipt
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center space-y-6">
        ...
        <div className="flex justify-center gap-3 pt-2">
          <Link
            href="/#offers"
            className="px-6 py-3 bg-[#3f2911] text-white rounded-xl text-xs font-bold hover:bg-[#2a1a0a] transition"
          >
            عروض وبكجات التوفير
          </Link>
          <Link
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        {/* RIGHT COLUMN (RTL Left): Cart Items & Order Summary */}
          ...
          <div className="luxury-card p-5 bg-white border border-slate-200 space-y-4">
            ...
            <div className="space-y-3 divide-y divide-slate-100">
              {items.map((item) => (
                <div key={item.id} className="pt-3 first:pt-0 flex gap-3 items-start justify-between">
                  <div className="relative w-16 h-16 rounded-xl bg-slate-50 border border-slate-100 shrink-0 overflow-hidden p-1 flex items-center justify-center">
                    <Image
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-slate-500">
                      ...
                      {item.surchargeTotal > 0 && (
                        <span className="text-amber-700 font-bold">
                          (+{item.surchargeTotal} د.أ صنف فاخر)
                        </span>
                      )}
                    </div>

                    {/* Bundle Sub-items breakdown */}
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <div className="flex items-center gap-1.5 bg-slate-100 rounded-lg p-0.5">
                        <button
                          ...
                          className="w-5 h-5 rounded bg-white text-slate-700 flex items-center justify-center text-xs font-bold hover:bg-slate-200"
                        >
            ...
            <div className="pt-4 border-t border-slate-100 space-y-2 text-xs">
              ...
              <div className="flex justify-between text-slate-600">
                ...
                {shipping === 0 ? (
                  <span className="font-bold text-emerald-700">مجاني (عرض خاص)</span>
                ) : (
                  <span className="font-bold text-slate-900">{shipping} د.أ</span>
                )}
              </div>
              <div className="flex justify-between text-sm font-extrabold text-[#3f2911] pt-2 border-t border-slate-200">
                <span>المجموع الكلي عند الاستلام:</span>
                <span className="text-lg">{total} د.أ</span>
              </div>
            </div>
        ...
        {/* LEFT COLUMN (RTL Right): Direct 4-Field Guest Checkout Form */}
        <div className="lg:col-span-6 order-1 lg:order-2">
          
          <form
            onSubmit={handleSubmitOrder}
            className="luxury-card p-6 sm:p-8 bg-white border border-[#ba997a]/40 shadow-xl space-y-5"
          >
            <div>
              <h2 className="text-base sm:text-lg font-extrabold text-[#3f2911]">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm font-medium outline-none focus:bg-white focus:border-[#ba997a]"
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {phoneValidation.isValid && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded">
                    ...
                    رقم أردني صحيح ({phoneValidation.carrier})
                  </span>
                )}
              </div>
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-3 bg-slate-50 border rounded-xl text-xs sm:text-sm font-bold text-left outline-none transition ${
                    phoneValidation.isValid
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm font-bold outline-none focus:bg-white focus:border-[#ba997a]"
                >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm font-medium outline-none focus:bg-white focus:border-[#ba997a]"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:bg-white focus:border-[#ba997a]"
              />
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl text-white text-sm sm:text-base font-extrabold shadow-lg transition-all flex items-center justify-center gap-2 ${
                isSubmitting
