...
import { ShoppingBag, ArrowRight, Truck, Check, ShieldCheck, Clock, Wind, Award, Zap } from "lucide-react";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12 pb-24">
      ...
      <div>
        <Link
          href="/catalog"
          className="inline-flex items-center gap-2 text-xs font-bold text-slate-600 hover:text-[#ba997a] transition"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div className="lg:col-span-6 space-y-4">
          <div className="relative aspect-square w-full rounded-3xl bg-white border border-slate-200 shadow-lg p-6 overflow-hidden flex items-center justify-center">
            <Image
            ...
            />
            {surcharge > 0 && (
              <div className="absolute top-4 right-4 px-3 py-1 bg-amber-50 border border-amber-200 text-amber-800 text-xs font-bold rounded-xl shadow-xs">
          ...
          <div className="grid grid-cols-4 gap-3">
            {galleryImages.map((img, idx) => (
              <button
                ...
                className={`relative aspect-square rounded-2xl bg-white border p-1.5 transition-all overflow-hidden cursor-pointer ${
                  activeImageIdx === idx
        ...
        <div className="lg:col-span-6 space-y-6 text-right">
          ...
          <div className="flex flex-wrap gap-1.5">
            {product.categories.map((c) => (
              <span key={c} className="text-[11px] font-bold px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700">
                {c}
              </span>
            ))}
          </div>
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-[#3f2911] leading-tight">
              {product.name}
            </h1>
            <p className="text-sm font-semibold text-slate-500 mt-1" dir="ltr">
              {product.nameEn}
            </p>
          </div>

          {/* Pricing Row */}
          <div className="p-4 rounded-2xl bg-[#fdfbf7] border border-[#ba997a]/30 flex items-center justify-between">
            <div>
              <span className="text-xs text-slate-500 block">السعر الحالي ({selectedSize})</span>
              <span className="text-3xl font-extrabold text-[#3f2911]">
                {currentPrice} <span className="text-sm font-bold text-slate-600">د.أ</span>
              </span>
            </div>
            <div className="text-left">
              ...
              <span className="text-[11px] font-extrabold text-emerald-700">وفر 50% مع كافيل</span>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center justify-between">
                  <strong className="text-xs font-extrabold text-slate-900">حجم 110 مل (الأكثر طلباً)</strong>
                  {is110 && <Check className="w-4 h-4 text-[#ba997a]" />}
                </div>
                <span className="text-xs font-bold text-[#3f2911] mt-2">
              ...
              >
                <div className="flex items-center justify-between">
                  <strong className="text-xs font-extrabold text-slate-900">حجم 55 مل (حجم عملي)</strong>
                  {!is110 && <Check className="w-4 h-4 text-[#ba997a]" />}
                </div>
                <span className="text-xs font-bold text-[#3f2911] mt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
            <button
              ...
              className="py-3.5 px-6 rounded-xl bg-[#3f2911] hover:bg-[#2a1a0a] text-white text-xs sm:text-sm font-extrabold shadow-md transition flex items-center justify-center gap-2 cursor-pointer"
            >
            ...
            <button
              ...
              className="py-3.5 px-6 rounded-xl bg-white border border-[#ba997a] hover:bg-[#ba997a]/10 text-[#3f2911] text-xs sm:text-sm font-bold shadow-xs transition flex items-center justify-center gap-2 cursor-pointer"
            >
          ...
          <div className="luxury-card p-5 bg-white border border-slate-200 space-y-4">
            <h3 className="text-xs font-extrabold text-slate-900">مواصفات وأداء تركيبة كافيل:</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
              <div className="p-3 bg-slate-50 rounded-xl">
                <Clock className="w-5 h-5 mx-auto text-[#ba997a] mb-1" />
              ...
              <div className="p-3 bg-slate-50 rounded-xl">
                <Wind className="w-5 h-5 mx-auto text-[#ba997a] mb-1" />
              ...
              <div className="p-3 bg-slate-50 rounded-xl">
                <Zap className="w-5 h-5 mx-auto text-[#ba997a] mb-1" />
              ...
              <div className="p-3 bg-slate-50 rounded-xl">
                <Award className="w-5 h-5 mx-auto text-[#ba997a] mb-1" />
          ...
          {/* Upsell Banner */}
          <div className="p-4 rounded-2xl bg-gradient-to-r from-[#fdfbf7] to-white border border-[#ba997a]/40 flex items-center justify-between gap-3 shadow-xs">
            ...
            <Link
              href="/#offers"
              className="px-4 py-2 bg-[#3f2911] text-white text-xs font-bold rounded-xl shrink-0 hover:bg-[#2a1a0a] transition"
            >
              شاهد العروض
            </Link>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200 p-3 sm:hidden shadow-lg flex items-center justify-between gap-3">
        <div>
          ...
          <span className="text-lg font-extrabold text-[#3f2911]">
            {currentPrice} <span className="text-xs font-normal">د.أ</span>
          </span>
        </div>
        <div className="flex gap-2">
          <button
            type="button"
            onClick={handleAddToCart}
            className="px-3.5 py-2.5 rounded-xl bg-white border border-[#ba997a] text-[#3f2911] text-xs font-bold"
          >
            <ShoppingBag className="w-4 h-4" />
          </button>
          <button
            type="button"
            onClick={handleBuyNow}
            className="px-5 py-2.5 rounded-xl bg-[#3f2911] text-white text-xs font-bold"
          >
            اطلب الآن
          </button>
        </div>
